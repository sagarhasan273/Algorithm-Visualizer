...
function PolygonWithCenteredText({
  ...
  return (
    <g>
      <polygon
        points={points}
        fill={mid ? 'rgb(0, 189, 9)' : 'none'}
        stroke="black"
        strokeWidth={0.1}
      />
      <text x={centerX} y={centerY} style={styleText} textAnchor="middle" fill={mid ? 'white' : 'black'}>
        {value}
      </text>
      ...
      {left ? (
        <text x={centerX - 8} y={centerY} style={styleTextI} textAnchor="middle" fill="black">
          low
        </text>
      ) : null}
      {right ? (
